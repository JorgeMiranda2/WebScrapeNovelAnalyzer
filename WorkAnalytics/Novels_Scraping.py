...
class Novels_Scraping(Base_Scraping):
    ...
    def execute(self):
        try:
            ...
            print("Going to list...")
            self.go_to_list()
            print("Going to sections list...")
            self.browse_sections_list()
            print(self.data)
            Convert_To_Csv(self.data, "Novels.csv")
        except Exception as e:
            print("An error has ocurred in the process: " , e)
        finally:
            self.driver.close()
            return self.data
...
# Resto de tu código
    ...
    def login_verifier(self):
        try:
            e = self.wait.until(ec.presence_of_element_located((By.XPATH, "//span[@id='menu_right_item']")))
            return True  
        except Exception as e:
            print(f"Error: {e}")
            return False    
